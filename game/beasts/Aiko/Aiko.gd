...
# state 
var max_hp = max_health_ratio * Constants.AIKO_MAX_HP
var max_energy = max_energy_ratio * Constants.AIKO_MAX_ENERGY
var hp = max_hp
var energy = 100.0
...
@onready var body: AikoBody = $Body
...
func consume_energy(delta: float) -> void:
	var energy_cost = 0.2 * delta # metabolic rate
	if body.is_moving:
		energy_cost += 1.2 * delta
