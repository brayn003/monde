...
func add_link() -> void:
	...
	for _try in Params.num_tries_find_link:
		...
		if (not (to == from or to.neuron_type in [0, 1]) and
			not (Params.no_feed_back and (to.position.x < from.position.x)) and
...
func add_direct_link() -> void:
	...
	"""
	for _try in Params.num_tries_find_link:
		var input_neuron = neurons.values()[Utils.random_i_range(1, Params.num_inputs)]
		var output_neuron = neurons.values()[Utils.random_i_range(Params.num_inputs + 1,
...
func clone(g_id: int) -> Genome:
	...
	var _clone = get_script().new(g_id,
								 clone_neurons,
								 clone_links)
