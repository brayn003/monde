...
# whether the genome is currently active
var is_active := false
...
func mutate(mutation_rate: int) -> void:
	...
	"""
	if Utils.random_f() < _params.prob_add_link[mutation_rate]:
		add_link()
	if Utils.random_f() < _params.prob_loop_link[mutation_rate]:
		add_loop_link()
	if Utils.random_f() < _params.prob_direct_link[mutation_rate]:
		add_direct_link()
	if Utils.random_f() < _params.prob_disable_link[mutation_rate] and not links.is_empty():
		disable_link()
	if Utils.random_f() < _params.prob_add_neuron[mutation_rate] and not links.is_empty():
		add_neuron()
...
func add_link() -> void:
	...
	"""
	for _try in _params.num_tries_find_link:
		...
		if (not (to == from or to.neuron_type in [0, 1]) and
			not (_params.no_feed_back and (to.position.x < from.position.x)) and
			not (from.position.x == 0 and to.position.x == 0) and
			not (from.position.x == 1 and to.position.x == 1)):
...
func add_direct_link() -> void:
	...
	"""
	for _try in _params.num_tries_find_link:
		var input_neuron = neurons.values()[Utils.random_i_range(1, _params.num_inputs)]
		var output_neuron = neurons.values()[Utils.random_i_range(_params.num_inputs + 1,
														_params.num_inputs + _params.num_outputs)]
...
func add_neuron() -> void:
	...
	for _try in _params.num_tries_find_link:
		...
		if (split_link.enabled and
			...
			not from.neuron_type == Constants.NeuronType.BIAS):
			# check for chaining if the number of HIDDEN neurons is below threshold
			if (_params.prevent_chaining and
				neurons.size() - _params.num_inputs - _params.num_outputs-1 < _params.chain_threshold):
	...
	var split_neuron_id = split_data[Constants.SplitMembers.NEURON_ID]
	var link1_innov_id = split_data[Constants.SplitMembers.FROM_LINK]
	var link2_innov_id = split_data[Constants.SplitMembers.TO_LINK]
	...
	var split_neuron = Neuron.new(
		split_neuron_id,
		Constants.NeuronType.HIDDEN,
		pos,
		_params.default_curve,
		false)
...
func mutate_weights(mutation_rate: int) -> void:
	...
	for link in links.values():
		if Utils.random_f() < _params.prob_weight_mut[mutation_rate]:
			if Utils.random_f() < _params.prob_weight_replaced[mutation_rate]:
				link.weight = Utils.gauss(_params.w_range)
			else:
				link.weight += Utils.gauss(_params.weight_shift_deviation)
...
func mutate_activation_response(mutation_rate: int) -> void:
	...
	for neuron in neurons.values():
		if Utils.random_f() < _params.prob_activation_mut[mutation_rate]:
			neuron.activation_curve += Utils.gauss(_params.activation_shift_deviation)
...
func crossover(mate: Genome, g_id: int) -> Genome:
	...
	for n_id in best.neurons.keys():
		if best.neurons[n_id].neuron_type != Constants.NeuronType.HIDDEN:
	...
	for link_id in best.links.keys():
		if best.links[link_id].enabled:
			...
			if worst.links.has(link_id) and Utils.random_f() < _params.gene_swap_rate:
...
func get_compatibility_score(other_genome: Genome) -> float:
	...
	if my_innovs.is_empty() and other_innovs.is_empty():
		return _params.species_boundary - 1
	...
	var distance = (
		(_params.coeff_matched * weight_difference) / num_matched +
		(_params.coeff_disjoint * num_disjoint) / longest +
		(_params.coeff_excess * num_excess) / longest)
	return distance
