...
func _draw_body() -> void:
	...
	var body_position = position.rotated(rotation)
	var tail_position = (position - Vector2((_body_radius + tail_radius), 0)).rotated(rotation)
...
func _draw_eyes() -> void:
	...
	var eye_positions = [
		position.rotated(rotation) + Vector2(_body_radius, 0).rotated(rotation + (_vision_angle / 2)),
		position.rotated(rotation) + Vector2(_body_radius, 0).rotated(rotation - (_vision_angle / 2)),
	]
